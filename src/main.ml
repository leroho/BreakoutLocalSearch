...
let bls = fun graph t l0 lmax->
  ...
  let fc_array = ref [|(0, (int_of_float !fc) * 10)|] in
  let cbest = ref !c in
  let fbest = ref !fc in
  let cp = ref !c in
  let w = ref 0 in
  ...
  let alpha_r =0.9 and alpha_s=0.1 and p0 = 0.4 in
  ...
  while !nbIter < 1000 do
    while (not (Pqueue.is_empty !pa)) || (not (Pqueue.is_empty !om) && (let (prio, (u, v), reste_om) = Pqueue.extract ~cmp !om in prio)>0.) do
      if Pqueue.is_empty !pa then
		...
		Move.apply_move m graph c pa om fc !nbIter tl);
	  fc_array:= Array.append !fc_array [|(!nbIter, (int_of_float !fc) * 10)|];
	  nbIter := !nbIter + 1
    done;
	Draw.draw graph !c;
  ...
  Printf.printf "durrée : %f/n" (time_end -. time_start);
  Printf.printf "fbest: %f" !fbest;
  !cbest;;
